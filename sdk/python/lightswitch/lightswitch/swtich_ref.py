...
# type hinting - JSON 데이터의 구조와 기대되는 값의 타입을 명확하게 지정
...
class Flagsmith:
    ...
    def __init__(
        self,
        environment_key: typing.Optional[str] = None, # API key
        api_url: typing.Optional[str] = None, # default url을 override할 url
        realtime_api_url: typing.Optional[str] = None, # real-time default url을 override할 url
        custom_headers: typing.Optional[typing.Dict[str, typing.Any]] = None, # 플래그 서버에 api요청을 보낼 때, 헤더에 추가로 담을 내용
        request_timeout_seconds: typing.Optional[int] = None, # 요청에 대한 응답 대기 시간
        enable_local_evaluation: bool = False, # 플래그 값을 로컬에서 정하는 것이 가능한지 여부(원격 저장소 값과 별개로)
        environment_refresh_interval_seconds: typing.Union[int, float] = 60,
        retries: typing.Optional[Retry] = None, # 요청 실패시 재시도 여부 
        enable_analytics: bool = False,  
        default_flag_handler: typing.Optional[
            typing.Callable[[str], DefaultFlag]
        ] = None, # 원격 저장소에서 플래그 데이터를 가져오지 못했거나 존재하지 않는 플래그에 대한 요청의 경우 호출될 함수
        proxies: typing.Optional[typing.Dict[str, str]] = None,
        offline_mode: bool = False,
        offline_handler: typing.Optional[BaseOfflineHandler] = None, # offline mode일 때 다른 저장소에서 환경 정보를 가져오는 핸들러
        enable_realtime_updates: bool = False, # polling 대신 SSE를 사용할 것인지 여부
    ):
        """
        ...
        if not self.offline_mode:
            ...
            if self.enable_local_evaluation:
                ...
                # 서버 키가 유효한 경우, local evaluation을 위한 설정 초기화
    ...
    # 새로운 SSE event를 받았을 때 EventStreamManager 스레드에 의해 호출됨
    def handle_stream_event(self, event: StreamEvent) -> None:
        ...
        # 새로 받은 이벤트의 타임스탬프가 로컬 환경 업데이트 타임스탬프보다 더 최신인 경우
        if stream_updated_at > environment_updated_at:
            self.update_environment() # 가장 최근의 환경 데이터를 fetch

    # 현재 환경의 플래그 데이터를 모두 담고 있는 Flags object를 반환
    def get_environment_flags(self) -> Flags:
        ...
        if (self.offline_mode or self.enable_local_evaluation) and self._environment:
            return self._get_environment_flags_from_document() # 오프라인 모드거나 local evaluation 모드이면 로컬에서 데이터 가져옴 
        return self._get_environment_flags_from_api() # 그렇지 않다면 원격 저장소에서 api 이용하여 데이터 가져옴 
    # identity를 인자로 받아 해당 identity의 플래그 정보를 모두 가져옴
    def get_identity_flags(
        self,
        identifier: str, # 이메일, 유저네임 등 사용자를 식별할 수 있는 정보
        ...
        return self._get_identity_flags_from_api(identifier, traits) # 오프라인 모드거나 local evaluation 모드가 아닌 경우라면 원격 저장소에서 데이터 가져오기
    ...
    # local environment data를 원격저장소에서 데이터를 가져와 업데이트
    ...
    # fetches the environment data from the Flagsmith API
    def _get_environment_from_api(self) -> EnvironmentModel:
        # api요청을 보내고 필요한 데이터를 JSON타입으로 받음
    ...
    # retrieves the feature flags from the local environment data
    ...
    def _get_identity_flags_from_api(
        ...
        try:
            data = generate_identities_data(identifier, traits) # 인자로 받은 데이터를 가지고 요청 시 body에 담아서 보낼 data 생성
    ...
    def _get_json_response(
        ...
        try:
            # self.session에서 method.lower() 변수의 값 가져오기 
    ...
    def _get_identity_model(
        ...
        **traits: TraitValue,
        ...
        # identifier가 _identity_overrides_by_identifier에 존재하는 경우 identity에 해당 식별자를 할당
        if identity := self._identity_overrides_by_identifier.get(identifier):
            identity.update_traits(trait_models)
            return identity
        # 존재하지 않는 경우 해당 identifier로 새로운 IdentityModel 생성
    ...
    def __del__(self) -> None:
        #"environment_data_polling_manager_thread"라는 멤버가 있는지 확인
        if hasattr(self, "environment_data_polling_manager_thread"): 
            self.environment_data_polling_manager_thread.stop() # 있다면 해당 thread를 중지
